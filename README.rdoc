...
Also notice that next/previous buttons are not included by default. If you want buttons you need override the <tt>buttons</tt> option. As in the example above, you'll need to set a simple javascript object with <tt>next</tt> and <tt>previous</tt> keys. The values will be used as text for your buttons.
